...
void TableArr::generate_text(float a, char prior[] = NULL, uint textSize=1000) {
  char context[k];
  float random;
  float prob;
  uint id;
...
void FCM::generate_text(float a, char prior[],uint textSize) { table->generate_text(a, prior,textSize); }
