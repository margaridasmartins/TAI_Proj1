...
void TableArr::generate_text(){
  ...
  for(uint i=0;i<1000;i++){
      prob=0;
      id=get_index(context);
      random=(float)rand()/(float)RAND_MAX; //generate target probability

      if(table[id][0]!=0){
        for(auto pair:alphabet){
          prob+=(float)(table[id][alphabet[pair.first]+1]+a)/(float)(table[id][0]+a*alphabet.size());
          if(prob>random){
              printf("%c",pair.first);
              for(uint j =0; j<k-1;j++){
                  context[j]= context[j+1];
              }
              context[k-1]=pair.first;
              break;
          }
        }
      }else{
...
void TableHash::generate_text(){
  ...
  for(uint i=0;i<1000;i++){
    ...
    if(table.find(context)!=table.end()){
      for(auto pair:table[context].occorrencies){
        prob+=(float)(pair.second+a)/(float)(table[context].sum+a*symbols.size());
        if(prob>random){
            printf("%c",pair.first);
            for(uint j =0; j<k-1;j++){
                context[j]= context[j+1];
